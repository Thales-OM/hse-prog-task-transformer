...
class PromptBuilder:
    ...
    def build_multichoice(cls, question: Question) -> str:
        if question.type not in QUESTION_MULTICHOICE_TYPES:
            raise ValueError(f"Question of type '{question.type}' wrongly passeed to Multichoice prompt build method")
    ...
    def build_coderunner(cls, question: Question) -> str:
        if question.type not in QUESTION_CODERUNNER_TYPES:
            raise ValueError(f"Question of type '{question.type}' wrongly passeed to Coderunner prompt build method")
        correct_answers = [answer.text for answer in question.answers] # Coderunner only contains correct answers
        test_cases = [f"[{idx+1}] Входные данные:\n{test_case.input}\nОжидаемый вывод:\n{test_case.expected_output}\n\n" for idx, test_case in enumerate(question.test_cases)]
    ...
    def build_other(cls, question: Question) -> str:
        if question.type in QUESTION_MULTICHOICE_TYPES:
            raise ValueError(f"Question of type '{question.type}' wrongly passeed to Other prompt build method. Must be passed to Multichoice builder.")
        if question.type in QUESTION_CODERUNNER_TYPES:
            raise ValueError(f"Question of type '{question.type}' wrongly passeed to Other prompt build method. Must be passed to Coderunner builder.")
...
def construct_messages(question: Question) -> List[dict]:
    return [
        {"role": "system", "content": "Ты опытный преподаватель Python для студентов гуманитарных специальностей"},
        {"role": "user", "content": PromptBuilder.build(question=question)}
    ]


async def get_completion(client: AsyncClient, model: str, messages: List[dict], temperature: float = DEFAULT_MODEL_TEMPERATURE) -> ChatCompletion:
...
async def make_inference(client: AsyncClient, model_id: int, question_id: int, cursor: cursor, temperature: float = DEFAULT_MODEL_TEMPERATURE) -> int:
    ...
    return await create_inference(question_id=question_id, model_id=model_id, inference=model_response, cursor=cursor)
