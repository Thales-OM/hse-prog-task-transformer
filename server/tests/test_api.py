...
def test_upload_quiz_xml(test_client):
    response = test_client.post(
        "/quiz/xml", content=SAMPLE_XML, headers={"Content-Type": "application/xml"}
    )
...
def test_inference_workflow(test_client, mock_openai_client):
    ...
    # Create inference
    inference_data = {"question_id": 1, "model_id": 1, "temperature": 0.5}
...
def mock_openai_client(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.api.routes.upload.openai.AsyncClient", MagicMock(return_value=mock_client)
    )
    return mock_client
